...
module.exports = function createThrottler (config, limited) {
  ...
  function errHandler (res, next, error) {
    ...
    // SEND A EMAIL POTENTIALLY IF SOMEHTING IS THAT BROKEN OR IF WE SENSE A PEN TEST OF 10000+ REQUESTS
    if (error) {
      res.statusCode = 500
      next(error)
      return true
  ...
  function throttleMiddleware (request, response, next) {
    ...
    ip = ip.split(',')[0]
    if (ip.substr(0, 7) === '::ffff:' || ip === '::1') {
      // DEFUALT TO LOCALHOST SO THAT IF WE DO GET A INVALID IP WE CAN RELY ON THE WE CAN STILL LIMIT THE ATTACK OR PERSON
      ip = '127.0.0.1'
    }

    // POTENTIALLY ADD A REGEX CHECK
    // var ipRegex = require('ip-regex')
    // contains an IP address
    // ipRegex().test('unicorn 192.168.0.1')

    // WE NEED TO ADD A MONGOOSE DB CONNECTION CHECK - IT FROZE TESTING AND DID THROW ANY ERRORS IT JUST WAITED
    Throttle
